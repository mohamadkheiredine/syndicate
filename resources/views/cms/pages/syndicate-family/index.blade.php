        ...
        <table class="table align-items-center table-flush">
            <thead class="thead-light">
                <tr>
                    ...
                    <th scope="col" class="no-sort">Designation</th>
                    <th scope="col" class="no-sort">Year</th>
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        @if($row->main_image)
                        <img src="{{ $row->main_image }}" width="80" class="img-thumbnail">
                        @endif
                    </td>
                    <td>{{ $row->name }}</td>
                    <td>{{ $row->designation }}</td>
                    <td>{{ $row->syndicate_year }}</td>
                    <td>{{ $row->admin->full_name ?? '' }}</td>
                    <td class="adjust-element">
                        <label class="custom-toggle mb-0">
                            <input
                                @can('syndicate_family-publish') onclick="togglePublish({{ $row->id }})" @else disabled @endcan
