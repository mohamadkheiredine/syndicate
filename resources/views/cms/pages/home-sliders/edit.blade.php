...
@section('content')
@include('cms.layouts.headers.partials', ['title' => 'Edit Home Slider'])
    ...
    <div class="card bg-secondary shadow">
        ...
        <div class="card-body">
            ...
            <form method="post" action="{{ route('admin.'.$page_info['link'].'.update', $row) }}" enctype="multipart/form-data" autocomplete="off">
                ...
                {{-- Main Image --}}
                @include('cms.components.inputs.image', ['label' => 'Main Image', 'asterix' => true, 'name' => 'main_image', 'value' => $row->main_image, 'text' => 'Image size should be 1360x685 px'])

                {{-- Mobile Image --}}
                @include('cms.components.inputs.image', ['label' => 'Mobile Image', 'asterix' => true, 'name' => 'mobile_image', 'value' => $row->mobile_image, 'text' => 'Image size should be 720x1280 px'])
                ...
                {{-- Text --}}
                @include('cms.components.inputs.text', ['label' => 'Text', 'name' => 'text', 'row' => $row])

                @can('home_sliders-publish')
                {{-- Published --}}
