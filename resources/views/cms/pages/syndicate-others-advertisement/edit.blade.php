    ...
    <div class="card bg-secondary shadow">
        ...
        <div class="card-body">
            <form method="post" action="{{ route('admin.'.$page_info['link'].'.update', $row) }}" enctype="multipart/form-data" autocomplete="off">
                ...
                {{-- Location --}}
                @include('cms.components.inputs.select-single', ['label' => 'Location', 'asterix' => true, 'name' => 'position', 'placeholder' => '-Select Location-', 'rows' => [['id' => 1, 'title' => 'Right Up'], ['id' => 2, 'title' => 'Right Down'], ['id' => 3, 'title' => 'Home']], 'value_attribute' => 'id', 'attribute' => 'title', 'row' => $row])

                {{-- Image --}}
                @include('cms.components.inputs.image', ['label' => 'Image', 'asterix' => true, 'name' => 'main_image', 'value' => $row->main_image, 'text' => 'Image size should be 520x320 px for Home, 300x200 px for Right Up/Right Down.'])
