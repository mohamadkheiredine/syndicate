...
@section('content')
@include('cms.layouts.headers.partials', ['title' => 'Edit Advertisement'])
    ...
    <div class="card bg-secondary shadow">
        ...
        <div class="card-body">
            <form method="post" action="{{ route('admin.'.$page_info['link'].'.update', $row) }}" autocomplete="off">
                ...
                {{-- Advertisement Type --}}
                @include('cms.components.inputs.text', ['label' => 'Advertisement Type', 'asterix' => true, 'name' => 'advertise_type', 'maxlength' => 255, 'row' => $row])

                {{-- Title --}}
                @include('cms.components.inputs.text', ['label' => 'Title', 'asterix' => true, 'name' => 'title', 'maxlength' => 255, 'row' => $row])

                {{-- Dimension --}}
                @include('cms.components.inputs.text', ['label' => 'Dimension', 'asterix' => true, 'name' => 'dimension', 'maxlength' => 255, 'row' => $row, 'text' => 'e.g. 300x200 px'])

                {{-- Type --}}
                @include('cms.components.inputs.text', ['label' => 'Type', 'asterix' => true, 'name' => 'what_type', 'maxlength' => 255, 'row' => $row, 'text' => 'e.g. Still'])

                {{-- Price --}}
                @include('cms.components.inputs.text', ['label' => 'Price', 'asterix' => true, 'name' => 'price', 'maxlength' => 255, 'row' => $row])

                {{-- Duration --}}
                @include('cms.components.inputs.text', ['label' => 'Duration', 'asterix' => true, 'name' => 'duration', 'maxlength' => 255, 'row' => $row, 'text' => 'e.g. ONE MONTH!'])
