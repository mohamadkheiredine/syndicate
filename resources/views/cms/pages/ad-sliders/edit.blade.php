...
@section('content')
@include('cms.layouts.headers.partials', ['title' => 'Edit Ad Slider'])
    ...
    <div class="card bg-secondary shadow">
        ...
        <div class="card-body">
            ...
            <form method="post" action="{{ route('admin.'.$page_info['link'].'.update', $row) }}" enctype="multipart/form-data" autocomplete="off">
                ...
                {{-- Main Image --}}
                @include('cms.components.inputs.image', ['label' => 'Main Image', 'asterix' => true, 'name' => 'main_image', 'value' => $row->main_image])

                {{-- Text --}}
                @include('cms.components.inputs.text', ['label' => 'Text', 'asterix' => true, 'name' => 'text', 'maxlength' => 255, 'row' => $row])

                @can('ad_sliders-publish')
                {{-- Published --}}
